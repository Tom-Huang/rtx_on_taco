...
class NpzDataset(BaseDataset):
    ...
    def __init__(self, *args, skip_frames: int = 0, n_digits: Optional[int] = None, aux_lang_loss_window: int = 1, pretrain: bool = False, load_lang_embeddings: bool = False, **kwargs):  # type: ignore
        self.ep_ids_file = "ep_start_end_ids.npy"
        super().__init__(*args, **kwargs)
        self.skip_frames = skip_frames
        self.aux_lang_loss_window = aux_lang_loss_window
        self.pretrain = pretrain
        self.load_lang_embeddings = load_lang_embeddings
        if self.with_lang:
