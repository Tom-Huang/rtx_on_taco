...
def process_language(episode: Dict[str, np.ndarray], transforms: Dict, with_lang: bool) -> Dict[str, torch.Tensor]:
    ...
    if with_lang:
        lang_data = episode["language"]
        if isinstance(lang_data, np.ndarray):
            lang = torch.from_numpy(episode["language"])
            lang = lang.squeeze() if lang.dtype == torch.float32 else lang
...
def get_state_info_dict(episode: Dict[str, np.ndarray]) -> Dict[str, Dict[str, torch.Tensor]]:
    ...
    """
    state_info = {"robot_obs": torch.from_numpy(episode["robot_obs"])}
    scene_obs = episode.get("scene_obs")
    if scene_obs is not None:
        state_info["scene_obs"] = torch.from_numpy(scene_obs)
    return {"state_info": state_info}
