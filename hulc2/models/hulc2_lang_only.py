...
class Hulc2LangOnly(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        super(Hulc2LangOnly, self).__init__()
    ...
    def training_step(self, batch: Dict[str, Dict], batch_idx: int) -> torch.Tensor:  # type: ignore
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            if "lang" in self.modality_scope:
                lang_latent_goal = self.language_goal(dataset_batch["lang"])
                vis_latent_goal = self.visual_goal(perceptual_emb[:, -1])
            lang_kl, lang_act_loss, lang_mod_loss, lang_pp_dist, lang_pr_dist, lang_seq_feat = self.lmp_train(
                perceptual_emb, lang_latent_goal, dataset_batch["actions"], dataset_batch["state_info"]["robot_obs"]
            )
            vis_kl, vis_act_loss, vis_mod_loss, vis_pp_dist, vis_pr_dist, vis_seq_feat = self.lmp_train(
                perceptual_emb, vis_latent_goal, dataset_batch["actions"], dataset_batch["state_info"]["robot_obs"]
            )
            if "lang" in self.modality_scope:
                if not torch.any(dataset_batch["use_for_aux_lang_loss"]):
                    ...
                    if self.use_clip_auxiliary_loss:
                        lang_clip_loss += self.clip_auxiliary_loss(
                            lang_seq_feat, lang_latent_goal, dataset_batch["use_for_aux_lang_loss"]
                        )
            encoders_dict["lang"] = [lang_pp_dist, lang_pr_dist]
            encoders_dict["vis"] = [vis_pp_dist, vis_pr_dist]
            kl_loss += vis_kl + lang_kl
            action_loss += lang_act_loss + vis_act_loss
            total_loss += lang_mod_loss + vis_mod_loss
            batch_size["lang"] = dataset_batch["actions"].shape[0]
            batch_size["vis"] = dataset_batch["actions"].shape[0]
