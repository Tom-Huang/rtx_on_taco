...
def load_model(cfg):
    checkpoint = get_checkpoints_for_epochs(Path(cfg.train_folder), [cfg.checkpoint])
    ...
    transforms = load_dataset_statistics(data_module.root_data_path, data_module.root_data_path, data_module.transforms)
...
def rollout(env, model, goal, ep_len=500):
    ...
    for step in range(ep_len):
        ...
        k = imshow_tensor("rgb_static", obs["rgb_obs"]["rgb_static"] / 255., wait=60, resize=True)
